-- ============================================================
-- PIANO LEARNING APP - CLEAN MIGRATION
-- Smaže staré tabulky a vytvoří je znovu
-- ============================================================
...
-- ============================================================
-- SMAZAT STARÉ TABULKY (v pořadí kvůli foreign keys)
-- ============================================================

DROP TABLE IF EXISTS piano.piano_user_achievements CASCADE;
DROP TABLE IF EXISTS piano.piano_achievements CASCADE;
DROP TABLE IF EXISTS piano.piano_user_progress CASCADE;
DROP TABLE IF EXISTS piano.piano_lessons CASCADE;
DROP TABLE IF EXISTS piano.piano_user_stats CASCADE;
DROP TABLE IF EXISTS piano.piano_users CASCADE;

-- Drop funkce pokud existuje
DROP FUNCTION IF EXISTS piano.update_updated_at_column() CASCADE;
...
CREATE INDEX idx_user_stats_user_id ON piano.piano_user_stats(user_id);
CREATE INDEX idx_user_progress_user_id ON piano.piano_user_progress(user_id);
CREATE INDEX idx_user_progress_lesson_id ON piano.piano_user_progress(lesson_id);
CREATE INDEX idx_user_achievements_user_id ON piano.piano_user_achievements(user_id);
CREATE INDEX idx_lessons_published ON piano.piano_lessons(is_published);

-- ============================================================
-- TRIGGERY pro updated_at
-- ============================================================

CREATE FUNCTION piano.update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
